...
export default function ServerChannel({ channel, server, role }: ServerChannelProps) {
    ...
    return (
        <button
            ...
            <p className={cn(
                "line-clamp-1 text-sm font-semibold text-zinc-500 dark:text-zinc-400 hover:text-zinc-500 dark:group-hover:text-zinc-300 transition ",
                params.channelId === channel.id && 'text-primary dark:text-zinc-200 dark:hover:text-white'
            )}>
                {channel.name}
            </p>
